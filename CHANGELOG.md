...
### Changed
- Replaced simulator product-name allocation heuristics and the explain engine's flat CEL map with the exact-version Kubernetes DRA CEL evaluator, typed selector inputs, ordered `FirstAvailable` handling, same-node `ExactCount` selection, and fail-closed warning events for unsupported requests.
