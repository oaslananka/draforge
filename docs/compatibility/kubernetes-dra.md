...
| Consumable capacity | Alpha extension | Simulator support includes exclusive filtering plus a bounded shareable-device subset with request-policy rounding and allocation accounting |
...
| Simulator allocation | Partial | Supports existing DeviceClasses, class/request selectors, ordered `FirstAvailable`, default/`ExactCount`, `All` over complete latest-generation pools, scalar `MatchAttribute`, exclusive and shareable capacity requirements, bounded backtracking, health, complete device/share identity, one-node selection and the 32-result limit; remaining advanced features fail closed |
| Device health | Partial | Custom label fallback exists; native health status support is incomplete |
| Consumable capacity | Partial | The simulator applies defaults, `validValues`, and `validRange` policies, aggregates committed and in-plan use, and writes UUID `ShareID` plus complete `ConsumedCapacity`; empty-capacity shareable devices and semantics outside this documented subset fail closed |
...
2. **Simulator allocation is not scheduler-equivalent.** It supports
   ...
   candidates by fixed quantity without producing sharing status. Devices with
   `allowMultipleAllocations=true` must define at least one capacity; the simulator
   applies policy defaults, rounds through `validValues` or `validRange`, aggregates
   committed and in-plan consumption by exact `<driver, pool, device>` identity, and
   records a unique UUID `ShareID` plus every `ConsumedCapacity` entry. Malformed,
   duplicate, incomplete, or overcommitted sharing status fails closed.
   `DistinctAttribute`, list-valued constraint semantics, admin access,
   taints/tolerations, partitionable/shared-counter devices, binding conditions and
   node-allocatable mappings remain unsupported. Unsupported input leaves the claim
   pending and emits a warning event; the simulator must not be used as a scheduler
   conformance oracle.
...
6. **Alpha/beta DRA extensions are not guaranteed.** The documented simulator
   subset covers consumable capacity, but v1.36-era extensions such as
   partitionable/shared-counter devices, device taints, binding conditions and node
   allocatable resources require explicit support before DRAForge can diagnose them
   accurately.
