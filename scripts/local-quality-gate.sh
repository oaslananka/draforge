...
echo "==> Total Go statement coverage: ${cov}%"
awk -v cov="$cov" 'BEGIN { if (cov + 0 < 80.0) { print "Error: Coverage is below 80.0%"; exit 1 } }'
...
scripts/verify-dependency-automation.sh
